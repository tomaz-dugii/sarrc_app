...
abstract class SupabaseTable<T extends SupabaseDataRow> {
  ...
  PostgrestFilterBuilder<PostgrestList> _select() => SupaFlow.client.from(tableName).select();
  ...
  Future<List<T>> querySingleRow({
    required PostgrestTransformBuilder Function(PostgrestFilterBuilder) queryFn,
      ...
      queryFn(_select()).limit(1).select().maybeSingle().then((r) => [if (r != null) createRow(r)]);

  Future<T> insert(Map<String, dynamic> data) => SupaFlow.client.from(tableName).insert(data).select().limit(1).single().then(createRow);
