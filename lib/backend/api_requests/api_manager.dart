...
import 'package:sarrc_app/utils/uploaded_file.dart';
...
class ApiManager {
  ...
  // have changed.
  static void clearCache(String callName) => _apiCache.keys.toSet().forEach((k) => k.callName == callName ? _apiCache.remove(k) : null);
  ...
  static Future<ApiCallResponse> multipartRequest(
    ...
    final nonFileParams = toStringMap(Map.fromEntries(params.entries.where((e) => !_isFile(e.value))));
